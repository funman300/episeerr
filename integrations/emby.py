...
class EmbyIntegration(ServiceIntegration):
        ...
        try:
            ...
            for item in items:
                ...
                if has_poster:
                    ...
                    raw_poster = f"{emby_url}/Items/{item_id}/Images/Primary?fillHeight=180&fillWidth=120&quality=90&api_key={api_key}"
                    poster = f"/api/integration/emby/art?url={_quote(raw_poster, safe='')}"
                else:
    ...
    def create_blueprint(self) -> Blueprint:
        ...
        @bp.route('/widget')
        def widget():
            """Return Emby Now Playing widget HTML."""
            try:
                config = integration.get_config()
                if not config:
                    return jsonify({'success': False, 'message': 'Emby not configured'})
                ...
                try:
                    ...
                    if resp.ok:
                        ...
                        for session in resp.json():
                            ...
                            if img_id:
                                ...
                                thumb = f"/api/integration/emby/art?url={_quote(raw_thumb, safe='')}"
                            else:
                                thumb = None
                            now_playing_data = {
                    ...
                    logger.warning(f"[Emby widget] Session fetch failed: {poll_err}")

                if not server_reachable:
                    return jsonify({'success': False, 'message': 'Emby unreachable'})

                logo = '<span style="font-size:10px;font-weight:700;color:#52B54B;flex-shrink:0;letter-spacing:0.5px;">Emby</span>'
                ...
                logger.error(f"Error generating Emby widget: {e}")
                return jsonify({'success': False, 'message': str(e)})

        @bp.route('/art')
        def art_proxy():
            """
            Server-side proxy for Emby poster/thumbnail art.
            Fetches image from the Emby server (raw HTTP) and streams it back
            to the browser over HTTPS, eliminating mixed content errors.
            Usage: /api/integration/emby/art?url=<encoded_emby_image_url>
            """
            ...
            try:
                ...
                logger.error(f"Emby art proxy failed for {decoded}: {e}")
